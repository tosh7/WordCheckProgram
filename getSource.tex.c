...
void closeSource()				 /*　ソースファイルと.texファイルをclose　*/
{
	fclose(fpi);
	fclose(fptex);
}
...
void printcToken()				/*　現在のトークンの印字　*/
{
	...
	if (i < end_of_KeyWd) 						/*　予約語　*/
		printf("{\\bf %s}", KeyWdT[i].word);
	else if (i < end_of_KeySym)					/*　演算子か区切り記号　*/
		printf( "%s", KeyWdT[i].word);
	else if (i==(int)Id){							/*　Identfier　*/
		switch (idKind) {
		case varId: 
			printf("%s", cToken.u.id); return;
		case parId: 
			printf("{\\sl %s}", cToken.u.id); return;
		case funcId: 
			printf("{\\it %s}", cToken.u.id); return;
		case constId: 
			printf( "{\\sf %s}", cToken.u.id); return;
		}
	}else if (i==(int)Num)			/*　Num　*/
		printf( "%d", cToken.u.value);
}
